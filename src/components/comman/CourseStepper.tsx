...
const CourseStepper = ({
  ...
  const handleChangeSteps = (index: number) => {
    if (courseData && +courseData?.course?.step >= +index) {
      if (!+courseId) {
        navigate(
          `/${pathName}/create_course?tab=${paramsTab}&step=${index}&id=${paramsId}&version=${paramsversion}`
        );
        onChangeStep?.(index?.toString());
      } else {
        if (+courseId) {
          navigate(
            `/${pathName}/create_course/${courseId}?tab=${paramsTab}&step=${index}&version=${paramsversion}${
    ...
    <div>
      <div className="mt-[-17px] flex items-center justify-between relative">
        {steps.map((step, index) => (
          <Fragment key={step}>
            <div
              className={`relative flex items-center justify-center rounded-full flex-col bg-transparent ${
                courseData && courseData?.course?.step >= index
                  ? "cursor-pointer"
                  : "cursor-default"
              } ${activeColor(index)}`}
            ...
            >
              ...
              <h3
                className={`w-full font-droid mt-[6px] text-sm sm:block hidden ${
                  currentStep && +currentStep === index
                    ? "text-[#00778B]"
                    : "text-grey"
                } ${
