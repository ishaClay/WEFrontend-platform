...
const EvaluateQuestionsDetailsItem = ({
  item,
  index,
}: EvaluateQuestionsDetailsItemProps) => {
  ...
  return (
    <div className="border border-solid border-[#D9D9D9] rounded-sm sm:p-5 p-3">
      <div className="flex items-center pb-3">
        <h6 className="sm:text-base text-sm font-droid font-bold pe-5 text-[#606060]">
          Question :<span className="ml-1">{index + 1}</span>
        </h6>
        <h6 className="sm:text-base text-sm font-droid font-bold text-[#606060]">
          Points :<span>{item?.question?.point}</span>
        </h6>
      </div>
      <p className="md:text-base sm:text-sm text-xs font-droid pb-3">
      ...
      <div className="">
        <ul className="flex flex-wrap flex-row md:gap-4 gap-2.5 items-center font-droid font-base">
          {item?.machedKeyword?.map((qItem, index) => {
            ...
            return (
              <li
                key={index}
                className={`text-${
                  answer?.split(", ")?.includes(qItem) ? "white" : "black"
                } py-1.5 text-center bg-[${
                  answer?.split(", ")?.includes(qItem) ? "#58BA66" : "#EDF0F4"
                }] px-5 rounded-full sm:text-base text-sm md:min-w-[104px] min-w-[95px]`}
              >
                {qItem}
              </li>
            );
