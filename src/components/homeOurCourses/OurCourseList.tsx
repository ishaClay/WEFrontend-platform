...
import { CoursePublishAdminClientData, CourseTime, IsOnline } from "@/types/allcourses";
        ...
        <div className="grid grid-cols-5 h-[104px]">
          <div className="gap-2 col-span-2">
            ...
            <div className="flex items-center gap-1 mb-[2px]">
              ...
              />
              <p className="text-xs leading-[22px] text-[#3A3A3A]">
              {data?.time === CourseTime.FullTime && (
                    <span>Full-time</span>
                  )}
                  {data?.time === CourseTime.PartTime && (
                    <span>Part-time</span>
                  )}
              </p>
            </div>
            <div className="flex items-center gap-1 mb-[2px]">
              ...
              <p className="text-xs leading-[22px] text-[#3A3A3A] sm:line-clamp-2 line-clamp-1">{data?.duration}</p>
            </div>
          </div>
          <div className="gap-2 col-span-3">
            ...
            <div className="flex items-center gap-1 mb-[2px]">
              ...
              <p className="text-xs leading-[22px] text-[#3A3A3A] sm:line-clamp-2 line-clamp-1">
              {data?.isOnline === IsOnline.Online && (
                    <span>Online</span>
                  )}
                  {data?.isOnline === IsOnline.InPerson && (
                    <span>InPerson</span>
                  )}
                  {data?.isOnline === IsOnline.Hybrid && (
                    <span>Hybrid</span>
                  )}
              </p>
            </div>
