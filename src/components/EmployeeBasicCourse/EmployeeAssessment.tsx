...
import { createEvalute, fetchAssesmentQuestion, fetchAssesmentSingleQuestion, fetchAssessmentScore } from "@/services/apiServices/assessment";
...
const schema = z.object({
    assesdmentAnswer: z.array(
        z.object({
            answer: z.union([
                z.string({ required_error: "Please select an answer" }).min(1, "Please select an answer"), // For single choice questions
                z.array(z.string({ required_error: "Please select an answer" })).nonempty("Please select an answer") // For multiple choice questions
            ])
        })
    )
});
...
const EmployeeAssessment = () => {
    ...
    const { data: assessmentScoreData, isPending: fetchAssessmentScorePending } = useQuery({
        queryKey: [QUERY_KEYS.getAssessmentScore, assessmentId],
        queryFn: () => fetchAssessmentScore(assessmentId as string, userData?.query?.detailsid as string),
        enabled: !!assessmentId
    });
        ...
        datasets: [
            {
                ...
                data: [assessmentScoreData?.data?.YourPercentage, 100 - Number(assessmentScoreData?.data?.YourPercentage)],
            ...
            <div className="flex items-center justify-between sm:p-[18px] p-[15px] bg-[#F3F3F3] border-b border-[#D9D9D9]">
                <div>
                    ...
                    <h5 className="text-[16px] font-[Calibri] text-[#606060]">
                        ...
                        {getAssessmentSingleQuestion?.data?.timeDuration?.hours?.toString()?.padStart(2, "0")} : {" "}
                        {getAssessmentSingleQuestion?.data?.timeDuration?.minutes?.toString()?.padStart(2, "0")} : {" "}
                        {getAssessmentSingleQuestion?.data?.timeDuration?.seconds?.toString()?.padStart(2, "0")} Second {" "}
                    </h5>
                </div>
            </div>
            {showAssessmentScore ? fetchAssessmentScorePending ? <span className="flex items-center justify-center py-10">
                ...
                <div className="flex items-center justify-center flex-col py-[75px]">
                    ...
                    <div className="mb-[38px]">
                        <h2 className="font-semibold mb-[5px]">Your Score : {assessmentScoreData?.data?.YourPercentage}% <span className="text-[#64A70B]">({assessmentScoreData?.data?.isPassed})</span></h2>
                        <p className="font-bold text-xs flex gap-[10px]">
                            <span className="text-[#64A70B]">Correct Answer : {assessmentScoreData?.data?.totalCorrect?.questions}</span>
                            <span className="text-[#FF5252]">Incorrect Answer : {Number(assessmentScoreData?.data?.total?.questions) - Number(assessmentScoreData?.data?.totalCorrect?.questions)}</span>
                        </p>
                    </div>
                    <p className="text-[#4285F4] cursor-pointer" onClick={() => navigate(`/employee/employee-basic-course/${state?.versionId}?courseId=${state?.courseId}`)}>Go Back</p>
                </div>
