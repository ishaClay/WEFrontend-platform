...
import { fetchAssignTo, fetchOneSupportTicket, getSingleSupportTicket, updateSupportTicket } from "@/services/apiServices/supportRequestServices";
...
const TicketsDetailsReply = () => {
  ...
  const { data: fetchSingleSupportTicket, refetch: refetchSingleSupport } = useQuery({
  ...
  const schema = z.object({
    ...
    details: z.string({ required_error: "Details is required" }).min(1, "Details is required"),
  })
  ...
  useEffect(() => {
    setValue("assignTo", String(fetchSingleSupportTicket?.data.data?.assignTo.id));
    setValue("ticketStatus", String(fetchSingleSupportTicket?.data.data?.status));
    setValue("details", String(fetchSingleSupportTicket?.data.data?.reply ? fetchSingleSupportTicket?.data.data?.reply : ""));
  ...
  const { mutate: updateTicket } = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [QUERY_KEYS.getSingleSupportTicket] });
      toast({
        variant: "default",
      ...
      <div className="pl-[20px] pr-[28px] mt-[26px]">
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="flex justify-between">
            <div className="flex gap-[11px]">
              <Avatar className="w-[32px] h-[32px]">
                ...
                <AvatarFallback>{userName(data?.data?.data?.openBy?.name)}</AvatarFallback>
              </Avatar>
              <div>
                <h3 className="text-[16px]">{data?.data?.data?.openBy?.name}</h3>
            ...
            <div className="flex gap-[28px]">
              <div>
                <span>Status: </span>
                <Button className={`rounded-[15px] ${data?.data?.data?.status === "Open" ? "bg-[#FFA25E]" : data?.data?.data?.status === "Answered" ? "bg-[#58BA66]" : "bg-[#0E9CFF]"}`}>
          ...
          <div className="max-w-full h-[320px] border solid 1px gray rounded-[10px] mt-[34px] p-[17px]">
            ...
            <p className="text-[#A3A3A3] text-[16px] mt-[18px]">Ticket Details</p>
            ...
            <div className="flex items-center mt-[32px]">
              ...
              <h3 className="text-[16px] ml-2">{data?.data.data?.documentUrl}</h3>
              <Button type="button" onClick={handleDownload} className="ml-[22px]">
                DOWNLOAD
              </Button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-[36px] mt-[29px]">
            ...
            <div>
              ...
              >
                ...
                <SelectContent>
                  {
                    fetchAssignToData?.data.data.filter((item: any) => item.name).map((item: any) => {
                      return <SelectItem key={item._id} value={String(item.id)}>{item.name}</SelectItem>
                    })
                  }
                </SelectContent>
              </Select>
            ...
            <div>
              ...
              {
                !errors?.ticketStatus?.ref?.value && <ErrorMessage message={errors?.ticketStatus?.message as string} />
              }
            </div>
          </div>
          ...
          <div className="w-full flex justify-end mt-[50px]">
            <Button type="submit" variant="secondary" className="w-[120px] h-[48px]">
              Submit
            </Button>
          </div>
        </form>
