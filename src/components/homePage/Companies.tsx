const Companies = () => {
  return (
    <div>
      <div className="xl:max-w-[1160px] max-w-full w-full mx-auto xl:px-0 px-5 mt-[67px]">
        <p className="button-color text-color inline-block xl:text-2xl text-xl rounded-[6px] font-semibold font-abhaya h-[47px] !leading-7 p-[10px]">
          For Companies
        </p>
        <div className="w-[784px] m-auto text-center xl:pt-[42px] pt-9 text-[#3A3A3A]">
          <h6 className="text-4xl font-normal font-abhaya leading-10">
            There’s never been
            <br />
            <span className="font-semibold">a clear road for you before</span>
          </h6>
          <p className="xl:pt-6 pt-5 xl:text-xl text-lg font-abhaya text-[#000000]">
            As a mindful, aware business: the goal for greater sustainability
            may always have been clear to you. But between not knowing where you
            even stand now, what actions you should take, and what progress even
            means—it’s a tall demand to expect you to take the steadfast
            journey.
            <br />
            <span className="text-[#479D5A]">
              But with the right support? We’d think it’d be a different story
              for you…
            </span>
          </p>
          <p></p>
        </div>
      </div>
    </div>
  );
};

export default Companies;
