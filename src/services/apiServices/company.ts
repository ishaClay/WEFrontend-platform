...
import { ProviderTypesType, TrainerByEmailType } from "@/types/Company";
import api from "./api";
...
export const getCompanyDetailsById = async ({
  ...
  const url = `api/v1/thirdparty/getCompany?companyNumber=${company_num}&companyName=${companyName}&clientId=${clientId}`;
  const method = "post";
...
export const fetchTrainerByEmailDataQuery = async (
  email: string
): Promise<TrainerByEmailType> => {
