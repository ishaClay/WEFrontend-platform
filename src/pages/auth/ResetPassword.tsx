...
//   const dispatch = useDispatch();
  ...
  const schema = z.object({
    ...
    password: z.string().max(8, "Password must be at least 8 characters")
                              .regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%&_.?]).{8}$/, "Password must contain at least one uppercase letter, one lowercase letter, and one special character"),
    ...
    <div className="flex h-screen 2xl:w-full">
        <div className="bg-[rgb(241,252,239)] w-1/2 font-poppins relative hidden flex-col items-center md:flex">
          ...
          <div className="absolute bottom-0 w-full">
            <img src={rePasswordBanner} alt="rePassword image" className="w-full" />
          </div>
        </div>
        ...
        <div className="lg:w-9/12 w-10/12 2xl:w-3/5">
          ...
          >
            ...
            <button onClick={() => navigate("/auth")} className="text-[14px] text-color font-medium mb-11 cursor-pointer">Login</button>
