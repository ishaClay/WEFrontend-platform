            ...
            <div className="flex flex-col  ">
                ...
                <div className="bg-[#FFFFFF] w-[1250px] h-[1469px] m-[12px] rounded-t-[10px]">
                    <div className="  pl-[30px] w-[1250px] h-[60px] bg-[#FFFFFF] border-b border-[#D9D9D9] rounded-t-[50px] flex items-center justify-between">
                        ...
                        <button className="bg-[#00778B] text-white px-4 py-2 rounded mr-[20px]  h-[45px] w-[150px]" >Send Invitation</button>
                    </div>
                        ...
                        <table className="table-auto w-full   ">
                            <thead>
                                <tr className="bg-[#F1F1F1] h-[50px]">
                                    ...
                                    <th className=" "> <span className="flex pl-4 ">ID <span className="mt-1"><TbSelector /></span></span></th>
                                    <th className=" "><span className="flex ml-4">Team Member Name<span className="mt-1"><TbSelector /></span></span></th>
                                    <th className=" "><span className="flex ml-4">Email Id<span className="mt-1"><TbSelector /></span></span></th>
                                    <th className=" "><span className="flex ml-4">Mobile Number<span className="mt-1"><TbSelector /></span></span></th>
                            ...
                            <tbody >
                                {employeeData.map(employee => (
                                    <tr key={employee.id} >
                                        ...
                                        <td className=" border-b px-18 py-4 "><span className="flex">    <img src="public/assets/img/face1.jpg" alt="Employee" className="w-8 h-8 rounded-full mr-2" />
                                            {employee.name}</span>
                                            {/* //https://via.placeholder.com/50 */}
                                        </td>
                                        <td className=" border-b px-18 py-2">{employee.email}</td>
                                        <td className="border-b px-18 pl-4 py-2">{employee.mobile}</td>
                                        <td className={`border-b px-18  py-2`}>
                                            <button className={`text-xs rounded-full ${employee.status === 'Registered' ? 'bg-[#00778B] text-white h-[32px] w-[80px]' : 'bg-[#0E9CFF] text-white h-[32px] w-[80px]'}`}>
                                                {employee.status}
                                            </button>
                                        </td>
                                        <td className={` border-b px-18 py-2  `}>
                                            <span className="flex">
                                                <button className={`${employee.action === 'Active' ? 'bg-green-500 text-white h-[32px] w-[80px] rounded-md' : 'bg-red-500 text-white h-[32px] w-[80px] rounded-md'}`}>
                                                    {employee.action}
                                                </button>
